...
// In production the same outputs come from the Python SAM 2 worker on a GPU; the frontend
...
// Background color = median of the photo border
const borderLab = (() => {
  const L = [], A = [], B = [];
  for (let x = 0; x < W; x += 3) for (const y of [0, H - 1]) { const p = y * W + x; const lab = toLab(rgb[p * 3], rgb[p * 3 + 1], rgb[p * 3 + 2]); L.push(lab[0]); A.push(lab[1]); B.push(lab[2]); }
  for (let y = 0; y < H; y += 3) for (const x of [0, W - 1]) { const p = y * W + x; const lab = toLab(rgb[p * 3], rgb[p * 3 + 1], rgb[p * 3 + 2]); L.push(lab[0]); A.push(lab[1]); B.push(lab[2]); }
  return [median(L), median(A), median(B)];
})();

// The model's mask often stops a pixel short of the garment outline, leaving a thin ring of the
// old color (very visible when recoloring dark fabric to white). Grow the mask onto neighbouring
// pixels that look more like the fabric than like the background.
function growToEdge(m, passes = 3) {
  ...
  for (let i = 0; i < passes; i++) {
    ...
    for (let p = W; p < n - W; p++) {
      if (m[p]) continue;
      const x = p % W;
      if (x === 0 || x === W - 1 || !(m[p - 1] || m[p + 1] || m[p - W] || m[p + W])) continue;
      const c = toLab(rgb[p * 3], rgb[p * 3 + 1], rgb[p * 3 + 2]);
      if (labDist(c, lab) < labDist(c, borderLab)) next[p] = 1;
    }
    m = next;
  }
  return m;
...
const top = growToEdge(refine(maskOf(['Upper-clothes', 'Dress'])));
const pants = growToEdge(refine(maskOf(['Pants', 'Skirt'])));
