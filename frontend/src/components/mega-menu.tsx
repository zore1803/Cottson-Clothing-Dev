...
export function MegaMenu() {
  ...
  return (
    <nav className="hidden items-center gap-1 md:flex" onMouseLeave={hide}>
      ...
      {open && (
        <div
          className="absolute left-1/2 top-full z-50 w-[min(560px,calc(100vw-2rem))] -translate-x-1/2"
          ...
          <div className="flex overflow-hidden rounded-b-md border bg-background shadow-2xl">
            {MENUS.find((m) => m.label === open)!.columns.map((col, i) => (
              <div key={col.heading} className={cn("min-w-0", col.cols === 2 ? "flex-[2]" : "flex-1", i > 0 && "border-l")}>
                <div className="border-b bg-muted/50 px-3.5 py-2 text-sm font-semibold text-brand">
                  {col.heading}
                  <span className="text-brand-accent">.</span>
                </div>
                <ul className={cn("grid gap-x-5 px-3.5 py-1", col.cols === 2 && "grid-cols-2")}>
                  {col.items.map((it) => (
                    <li key={it.title}>
                      <Link href={it.href} onClick={() => setOpen(null)} className="group block py-2">
                        <div className="text-xs font-semibold text-brand group-hover:underline">{it.title}</div>
                        <div className="truncate text-[11px] text-muted-foreground">{it.text}</div>
                      </Link>
                    </li>
                  ))}
                </ul>
